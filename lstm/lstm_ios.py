...
class LSTMModel(tf.keras.Model):
    def __init__(self, hidden_size, class_size):
        ...
        self.lstm = tf.keras.layers.LSTM(hidden_size, return_sequences=False, return_state=True,
...
#
# # 转换为 Core ML 模型
# coreml_model = ct.convert('lstm_model',
#                           inputs=[ct.TensorType(shape=(1, 25, 6)),
#                                   ct.TensorType(shape=(1, hidden_size)),
#                                   ct.TensorType(shape=(1, hidden_size))])
#
# # 保存 Core ML 模型
# coreml_model.save('LSTMModel.mlmodel')
...
# 创建模型实例
lstm_model = LSTMModel(hidden_size, num_classes)
...
def save_tflite(model, window_size, feature_size):
    ...
    # model directory.
    MODEL_DIR = "keras_lstm3_ios"
    model.save(MODEL_DIR, save_format="tf", signatures=concrete_func)

    converter = tf.lite.TFLiteConverter.from_saved_model(MODEL_DIR)
    tflite_model = converter.convert()
    # Save the model.
    with open('model_LSTM3_ios.tflite', 'wb') as f:
