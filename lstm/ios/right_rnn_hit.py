...
model_name = 'right_rnn_hit'
save_model_path_no_extension = save_model_base_path + os.sep + model_name

L2 = 0.000001

model = tf.keras.Sequential(name='sequential_1')
model.add(tf.keras.layers.SimpleRNN(32, return_sequences=False, input_shape=(window_size, feature_size), dropout=0.2))
...
model.add(tf.keras.layers.Dense(output_size, activation='softmax', kernel_regularizer=l2(L2), bias_regularizer=l2(L2),
                                name="Output"))
model.summary()
# tf.keras.utils.plot_model(model, to_file='model.png', show_shapes=True)  # 绘制计算图
