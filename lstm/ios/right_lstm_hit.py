...
batch_size = 128
num_epochs = 10
hit_data_path = '/Users/sinnus/WorkSpace/Lightweight-Transformer-Models-For-HAR-on-Mobile-Devices/lstm/train_hit_c25'
...
def save_tflite(model, window_size, feature_size, hidden_size):
    ...
    # 保存模型
    model.save(save_model_base_path + '/right_lstm_hit.keras')

    MODEL_DIR = save_model_base_path + "/right_lstm_hit"
    ...
    # 保存TFLite模型
    with open(save_model_base_path + '/right_lstm_hit.tflite', 'wb') as f:
        f.write(tflite_model)
    coreml_model = ct.convert([concrete_func])
    coreml_model.save(save_model_base_path + '/right_lstm_hit.mlmodel')
