...
# RealWorld,HHAR,UCI,SHL,MotionSense, COMBINED14
...
if (dataSetName == "COMBINED"):
    ...
    ACTIVITY_LABEL = ['Walk', 'Upstair', 'Downstair', 'Sit', 'Stand', 'Lay', 'Jump', 'Run', 'Bike', 'Car', 'Bus',
                      'Train', 'Subway']
    activityCount = len(ACTIVITY_LABEL)
    UCI = [0, 1, 2, 3, 4, 5]
    ...
    for index, datasetName in enumerate(datasetList):
        if (datasetName == 'UCI'):
            ...
            centralTrainLabelAligned.append(
                np.hstack([REALWORLD_CLIENT[labelIndex] for labelIndex in centralTrainLabel[index]]))
            centralTestLabelAligned.append(
                np.hstack([REALWORLD_CLIENT[labelIndex] for labelIndex in centralTestLabel[index]]))
            ...
            centralTrainLabelAligned.append(
                np.hstack([MotionSense[labelIndex] for labelIndex in centralTrainLabel[index]]))
            centralTestLabelAligned.append(
                np.hstack([MotionSense[labelIndex] for labelIndex in centralTestLabel[index]]))
        else:
    ...
    print(centralTrainData)
    print(centralTrainLabel)
    ...
    print(clientOrientationTrain)
    print(clientOrientationTest)
